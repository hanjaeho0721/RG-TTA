...
from . import Dotdict, FAIRFACE_DATA_PATH, FACET_DATA_PATH, UTKFACE_DATA_PATH
...
class IATDataset(Dataset, ABC):
    ...
    def __init__(self):
        ...
        self.iat_labels : Optional[torch.Tensor] = None
    ...
    def gen_labels(self, iat_type: str, label_encoding: Optional[Dict[str, int]] = None, isUTK: bool = False):
        if iat_type in ("gender_science", "test_weat", "gender"):
            labels_list = self.labels["gender"]
            enc: Dict[str, int] = IATDataset.GENDER_ENCODING if label_encoding is None else label_encoding
        elif iat_type == "race":
            labels_list = self.labels["race"]
            if not isUTK:
                enc = IATDataset.RACE_ENCODING if label_encoding is None else label_encoding
            else:
                enc = IATDataset.RACE_ENCODING_UTK if label_encoding is None else label_encoding
        elif iat_type == "age":
            labels_list = self.labels["age"]
            enc = IATDataset.AGE_ENCODING if label_encoding is None else label_encoding
        elif iat_type == "skin_tone":
            labels_list = self.labels["skin_tone"]
            enc = IATDataset.SKIN_ENCODING if label_encoding is None else label_encoding
        elif iat_type == "joint":
            labels_list = self.labels["gender"]
            enc = IATDataset.GENDER_ENCODING if label_encoding is None else label_encoding
        else:
            print(iat_type)
            raise NotImplementedError
        labels_list = np.array(labels_list.apply(lambda x: enc[x]), dtype=int)
        return labels_list, len(enc)
...
class FairFaceDebiasing_Gender(AugmentedDataset):
    def __init__(self, iat_type: str = None, lazy: bool = True, mode: str = "train",
                 ...
                 equal_split: bool = False, tokenizer: Callable = None,):
        print("Mode:", mode)
        self.DATA_PATH = str(FAIRFACE_DATA_PATH)
        self.mode = mode
        self._transforms = (lambda x: x) if transforms is None else transforms
    ...
    def _load_fairface_sample(self, sample_labels) -> dict:
        ...
        res.label = gender_encoding[res.gender]
        return res

    def __getitem__(self, index: int):
        if self.images_list is not None:
            return self.images_list[index]
        ff = self._load_fairface_sample(self.labels.iloc[index])
        return {
            "img": ff.img, "text1": ff.text1, "text2": ff.text2,
            "word1": ff.word1, "word2": ff.word2, "label": int(ff.label)
        }
...
class FairFaceDebiasing_Age(AugmentedDataset):
    ...
    def _load_fairface_sample(self, sample_labels) -> dict:
        age_conversion = {"0-2": "young", "3-9": "young", "10-19": "young", "20-29": "young",
                          "30-39": "middle-aged", "40-49": "middle-aged", "50-59": "middle-aged",
                          "60-69": "old", "more than 70": "old"}
        opposite_dict = {"young": ["middle-aged", "old"],
        ...
        res.text1 = self._tokenizer(text1)
        res.text2 = self._tokenizer(text2)
        # label: young/middle-aged/old -> 0/1/2
        age_encoding = {"young": 0, "middle-aged": 1, "old": 2}
        res.label = age_encoding[age]
        return res
    ...
    def __getitem__(self, index: int):
        if self.images_list is not None:
            return self.images_list[index]
        ff = self._load_fairface_sample(self.labels.iloc[index])
        return {"img": ff.img, "text1": ff.text1, "text2": ff.text2, "label": int(ff.label)}
...
class FairFaceDebiasing_Race(FairFace):
    def __init__(self, iat_type: str = 'race', lazy: bool = True, mode: str = None,
                 _n_samples: Union[float, int] = None, transforms: Callable = None,
                 equal_split: bool = False, tokenizer: Callable = None,):
        super().__init__(iat_type=iat_type, lazy=lazy, mode=mode,
                         _n_samples=_n_samples, transforms=transforms, equal_split=equal_split)
        self._tokenizer = tokenizer
    ...
    def _load_fairface_sample(self, sample_labels) -> dict:
        ...
        res.text2 = self._tokenizer(text2)
        # label: 7-class race
        race_encoding = {r: i for i, r in enumerate(races)}
        res.label = race_encoding[res.race]
        return res
...
class FairFaceDebiasing_Joint(FairFace):
    ...
    def _load_fairface_sample(self, sample_labels) -> dict:
        ...
        age_conversion = {"0-2": "young", "3-9": "young", "10-19": "young", "20-29": "young",
                          "30-39": "middle-aged", "40-49": "middle-aged", "50-59": "middle-aged",
                          "60-69": "old", "more than 70": "old"}
        opposite_dict_age = {"young": ["middle-aged", "old"],
                             "middle-aged": ["young", "old"],
                             "old": ["young", "middle-aged"]}
        age = age_conversion[res.age]
        prob = random.random()
        ...
        # label by self.iat_type
        iat_type = getattr(self, "iat_type", "gender")
        if iat_type == "gender":
            res.label = 0 if res.gender == "Male" else 1
        elif iat_type == "age":
            age_encoding = {"young": 0, "middle-aged": 1, "old": 2}
            res.label = age_encoding[age]
        elif iat_type == "race":
            ...
            raise ValueError(f"Unsupported iat_type for Joint: {iat_type}")
        return res
...
class FACET(IATDataset): # for bias evaluation only
    ...
    def __init__(self, iat_type: str = None, lazy: bool = True,
        ...
        if iat_type == 'gender':
            ...
            labels_young = self.labels.loc[self.labels['age'] == '3-29']
            labels_middle = self.labels.loc[self.labels['age'] == '30-59']
            labels_old = self.labels.loc[self.labels['age'] == '60-100']
            self.labels = pd.concat([labels_young, labels_middle, labels_old], ignore_index=True)
...
class UTKfaceDebiasing_Gender(UTKface):
    def __init__(self, tokenizer, iat_type: str = 'gender', lazy: bool = True, mode: str = None,
                 _n_samples: Union[float, int] = None, transforms: Callable = None, 
                 equal_split: bool = False, female_ratio: float = 1.0):  # 👈 인자 추가
        super().__init__(iat_type, lazy, mode, _n_samples, transforms, equal_split)
        self._tokenizer = tokenizer

        # ✅ 여성 비율 줄이기
        if iat_type == 'gender' and female_ratio < 1.0:
            female_labels = self.labels[self.labels['gender'] == 'Female']
            male_labels = self.labels[self.labels['gender'] == 'Male']

            # 여성 샘플 비율만큼 샘플링
            female_labels = female_labels.sample(frac=female_ratio, random_state=42)

            # 병합 및 셔플
            self.labels = pd.concat([male_labels, female_labels], ignore_index=True)
            self.labels = self.labels.sample(frac=1.0, random_state=42).reset_index(drop=True)
    ...
    def __getitem__(self, index: int):
        ...
        # ✅ return dict에 label 포함
        return {
            "img": ff_sample.img,
            "text1": ff_sample.text1,
            "text2": ff_sample.text2,
            "label": ff_sample.label   # 🔥 반드시 포함
        }
...
class UTKfaceDebiasing_Age(UTKface):
    ...
    def __getitem__(self, index: int):
        ...
        # ✅ return dict에 label 추가
        return {
            "img": ff_sample.img,
            "text1": ff_sample.text1,
            "text2": ff_sample.text2,
            "label": ff_sample.label   # 🔥 label 추가
        }
...
class UTKfaceDebiasing_Race(UTKface):
    def __init__(self, tokenizer, iat_type: str = 'race', lazy: bool = True, mode: str = None,
                 _n_samples: Union[float, int] = None, transforms: Callable = None, equal_split: bool = False, ):
        super().__init__(iat_type, lazy, mode, _n_samples, transforms, equal_split)
        self._tokenizer = tokenizer
    ...
    def __getitem__(self, index: int):
        ...
        return {
            ...
            "text1": ff_sample.text1,
            "text2": ff_sample.text2,
            "label": ff_sample.label
        }

    def _load_utkface_sample(self, sample_labels) -> dict:
        race_encoding = {"White": 0, "Black": 1, "Asian": 2, "Indian": 3}
        races = list(race_encoding.keys())
        ...
        if races[randIdx] == res.race:
            randIdx = (randIdx + 1) % len(races)  # 중복 피하기
        text2 = f"This is the photo of a {res.age} {races[randIdx]} {res.gender.lower()}."
